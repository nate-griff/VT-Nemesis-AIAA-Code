...
figure
yyaxis right
plot(time_sec, abs(aoa), 'LineWidth', 2.5, "Color", [30, 34, 36]/255, "LineStyle", ":")
hold on
conversion_factor = 0.737562; % Conversion factor from N-m to lb-ft

% plot(time3, tq1P3 * conversion_factor, "LineWidth", 2.5, "Color", [153, 0, 0]/255)
% plot(time3, tq2P3 * conversion_factor, "LineWidth", 2.5, "Color", [240, 64, 69]/255)
ylim([0,16])
x1 = [7 7];      
x2 = [11 11];    
yLimits = ylim;   
...
conversion_factor_thrust = 0.224809; % Conversion factor from N to lb

ylabel('Degrees', 'Interpreter', 'tex', 'FontSize', 22)
set(gca, 'FontSize', 18, 'FontName', 'Acherus Grotesque')  % Adjust the font size as needed
yyaxis left
plot(time3, abs(trval3 * 45 / max(trval) * conversion_factor_thrust), "LineWidth", 2.5, "Color", [142, 17, 23]/255)
plot(time3, smooth(abs(tqP3 * conversion_factor)*2.5), "LineWidth", 2.5, "Color", [15, 158, 214]/255, "LineStyle", "-") %[167, 177, 185]/255)
ylabel('lbf  |  lb-ft (x2.5)', 'Interpreter', 'tex', 'FontSize', 22, 'Color', 'black')
ylim([0,11])
set(gca, 'FontSize', 18, 'FontName', 'Acherus Grotesque')  % Adjust the font size as needed
ax = gca;
ax.YAxis(1).Color = 'black'; % Set the left y-axis ticks to black
ax.YAxis(2).Color = 'black'; % Set the right y-axis ticks to black
% plot([7 7], yLimits, 'k--', 'LineWidth', 1.5, 'HandleVisibility', 'off');
% plot([11 11], yLimits, 'k--', 'LineWidth', 1.5); 

title('Run 3: All Measurements', 'Interpreter', 'tex', 'FontSize', 25)
xlabel('Time (s)', 'Interpreter', 'tex', 'FontSize', 22)

legend({'Axial Thrust','Average Moment','Vane Angle of Attack'}, ...
       'Interpreter', 'tex' , 'FontSize', 22)
grid on

xlim([0,20])
set(gca, 'FontSize', 18, 'FontName', 'Acherus Grotesque')  % Adjust the font size as needed
exportgraphics(gcf, 'exp.png', ...
       'Resolution', 600);           % or higher for print-quality
...
% plot( AOA_interp,tr3M,'ko',AOA_interp,tr3M1,'b*',AOA_interp,tr3M2,'r>');
% legend('Average Moment','Moment Load Cell 1', 'Moment Load Cell 2')
% xlim([0,15])
% xlabel('Angle of Attack Degrees')
% ylabel('Moment (N-M)')

% figure
% plot(AOA_interp,tr3M2,'r>')
% legend('Load Cell 2')
% xlim([0,15])
% xlabel('Angle of Attack Degrees')
% ylabel('Moment (N-M)')
